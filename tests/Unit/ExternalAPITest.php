...
// Add similar tests for defer() and await()

// Test for defer() function inside run()
test('test defer() inside run()', function() {
    run(function() {
        $cleanupCalled = false;
        defer(function() use (&$cleanupCalled) {
            $cleanupCalled = true;
        });
        // Ensure the cleanup function is registered
        expect($cleanupCalled)->toBeFalse();
    });
});

// Test for defer() function outside run()
test('test defer() outside run()', function() {
    expect(function() {
        $cleanupCalled = false;
        defer(function() use (&$cleanupCalled) {
            $cleanupCalled = true;
        });
    })->toThrow(UsageError::class); // Expect UsageError when defer() is used outside run()
});

// Add similar tests for await() function
