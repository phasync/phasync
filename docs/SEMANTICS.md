...
| IO-6 *new* | `io::fgets` (removed, 2.0.0, see below) could return partial lines, and `io::fgetc` (removed) never waited | Moot: `phasync\io` is now `file_get_contents`/`file_put_contents`/`flock` only |
