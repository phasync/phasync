...
|---|----------|---------|-------------------|
| D1 | How does `read()` signal end-of-stream, given that `null` is also a legal value? | keep `null`; throw `ChannelClosedException`; add `receive(&$value): bool` like Go's `v, ok` | Keep `read()` returning `null` (iteration still works) and add `receive(&$value): bool` for code that must tell them apart |
| D2 | Which values can a channel carry? | keep the `Serializable` union; any `mixed` | `mixed`. The restriction only made sense for cross-process use |
...
| D17 | `readFixed($n, $timeout)` returns `null` on timeout, and also `null` at end-of-stream with too little data. `read()` now throws `TimeoutException` | keep `null`; throw `TimeoutException` like `read()`; distinguish the two cases | Open. `readFixed()`'s `null` is documented and two original tests rely on it, so nothing changed |
| D16 | Should `StringBuffer` get an optional maximum length? | none; opt-in `?int $maxLength = null` that makes `write()` throw when the unread bytes would exceed it | Open (maintainer: "we could consider"). Opt-in, throwing, default unbounded. Blocking the writer would change BUF-1 and add writer-side deadlocks |
